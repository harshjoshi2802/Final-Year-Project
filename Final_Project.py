...
def main():
    ...
    # st. set_page_config(layout="wide")
    page = st.sidebar.radio("Navigation Pane:", ["Product Recommendation using Market Basket Analysis",
                                                 "Customer Segmentation based on RFM Analysis", "Dashboard"])
    ...
    st.sidebar.markdown("Harsh Joshi and Ishan Singh")

    # Read clean dataset
    retail = pd.read_csv('C:/Users/harsh/Downloads/CleanRetailDataNew.csv')
    ...
    def group_sales_quantity(df, feature):
        df = df[[f'{feature}', 'Quantity', 'Sales Revenue']].groupby([f'{feature}']).sum().sort_values(
            by='Sales Revenue', ascending=False).reset_index()
        return df
    ...
    if page == "Product Recommendation using Market Basket Analysis":
        ...
        # Display
        AgGrid(country_retail, theme='alpine', height=200, width=150)
        ...
        # Opening JSON file
        with open('C:/Users/harsh/Downloads/item_sets.json') as json_file:
    ...
    if page == "Customer Segmentation based on RFM Analysis":
        ...
        try:
            ...
            with col1:
                ...
                st.markdown(
                    'It is a customer segmentation technique that uses past purchase behavior to segment customers. '
                    'To perform RFM analysis, we divide customers into four equal groups according to the '
                    'distribution of values for recency, frequency, and monetary value.')
            ...
            with col3:
                ...
                AgGrid(df, theme='alpine', height=200, width=150)
            ...
            # Pie charts
            rfm_stats = df[["Clusters", "RFM_Score", "Recency", "Frequency", "Monetary"]].groupby("Clusters").agg(
                ["mean"])
    ...
    if page == "Dashboard":
        ...
        top_customers = country_retail[country_retail["CustomerID"] != "Guest Customer"].groupby("CustomerID")[
            "InvoiceNo"].nunique().sort_values(ascending=False).reset_index().head(11)
        html_temp_title = """
        ...
        with col1:
            ...
            st.markdown(html_temp_title, unsafe_allow_html=True)
            st.markdown("###")
            top_products_qty = group_sales_quantity(country_retail, 'Description').sort_values(ascending=False,
                                                                                               by="Quantity").reset_index(
                drop=True)
            top_products_qty.drop('Sales Revenue', axis=1, inplace=True)
            # Display merged dataframes
            AgGrid(top_products_qty, theme='alpine', height=200, width=150)
        with col3:
            html_temp_title = """
            <div style="background-color:#ABBAEA;padding:4px">
            ...
            top_products_revenue = group_sales_quantity(country_retail, 'Description').sort_values(ascending=False,
                                                                                                   by="Sales Revenue").reset_index(
                drop=True)
            top_products_revenue.drop('Quantity', axis=1, inplace=True)
            # Display merged dataframes
            AgGrid(top_products_revenue, theme='alpine', height=200, width=150)
        ...
        with col1:
            ...
            country_data_wo_uk = retail[retail['Country'] != 'United Kingdom'].groupby('Country')[
                'InvoiceNo'].nunique().sort_values(ascending=False).reset_index()
        ...
        with col3:
            ...
            st.markdown(
                '- The retail store management can start posing question of why this is the case, especially when '
                'this is a Online retail store')
        ...
        with col1:
            ...
            st.markdown(html_temp_title, unsafe_allow_html=True)
            st.markdown("###")
            retail['Country'] = retail['Country'].astype(str)
            sales_wo_uk = retail[retail['Country'] != 'United Kingdom'].groupby('Country').sum().sort_values(
                by='Sales Revenue', ascending=False).reset_index()
        ...
        with col3:
            ...
            st.markdown("###")
            st.markdown(
                '- Total Quantity of items sold for Netherlands and Ireland(EIRE) are at top with values 190K and 140K')
        ...
        with col3:
            ...
            st.markdown("###")
            st.markdown(
                '- As expected, the Monthly statistics for November is the highest due to Thanksgiving Holiday/Black '
        ...
        with col3:
            ...
            st.markdown("###")
            st.markdown(
                "- Highest sales (by revenue and quantity) has been noted during the Fall season, which are the month "
                "of September, October, November")
            st.markdown(
                '- Lowest percentage of sales (by revenue and quantity) has been noted during the Winter season ('
                'January, February, March, April) where people are unable to leave the house due to harsh weather')
        ...
        with col3:
            ...
            st.markdown("###")
            st.markdown(
                "- People buy more on Thursdays to prepare for the weekend, followed by Tuesday to prepare for the "
                "week ahead")
